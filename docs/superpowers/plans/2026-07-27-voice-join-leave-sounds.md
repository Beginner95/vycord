...
- Produces:
  ...
  - `GroupCallCallbacks.onPeerLeft: (userId: string) => void` — сигнатура без изменений, но `groupCall.ts` теперь фильтрует `participant_left` с `user_id === this.currentUserId` до вызова колбэка (см. находку финального ревью: вытеснение stale-сессии при реконнекте/входе со второго устройства иначе рассылает `participant_left` с нашим же `user_id`, и колбэк даёт ложный «выход»). Только после этого фильтра `onPeerLeft` можно считать всегда означающим реальный уход.
...
- [ ] то же самое, но реконнект укладывается в 30 секунд (stale-сессия ещё жива на сервере): у переподключившегося участника **нет** ложного сигнала выхода в момент возврата в звонок;
- [ ] войти тем же аккаунтом со второго устройства/вкладки, пока первое ещё в звонке: на новом устройстве **нет** сигнала выхода (только свой вход);
