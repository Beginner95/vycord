...
- **Go** (`websocket_test.go` стиль): `handleConnectionQuality` — валидный payload
  даёт broadcast с `user_id` и метриками; битый JSON и неизвестный `level` — без
  паники и без рассылки.
- **Клиент**: юнит-тесты `computeQualityLevel` — все ветки и граничные значения
  (loss/rtt ровно на порогах, `hasData=false`).
