...
- `glitchtip-web` — Django/uvicorn, слушает порт (например
  ...
  наружу через nginx).
- `glitchtip-worker` — celery worker, обрабатывает события из очереди
  (обязателен — без него ingestion API складывает события в очередь, но они
  не сохраняются).
- `glitchtip-beat` — celery beat, периодические задачи GlitchTip (в т.ч.
  retention-очистка старых событий) — без него база событий будет расти
  неограниченно; тот же образ, что и `glitchtip-web`/`-worker`, отдельная
  команда запуска.
