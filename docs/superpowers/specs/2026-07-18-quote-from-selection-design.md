...
| Область охвата | Только desktop (мышь + клавиатура, включая Shift+стрелки для выделения). Touch/mobile не реализуется в этой задаче — на touch-устройствах браузер показывает своё нативное меню при выделении, которое конфликтовало бы с кастомным тултипом. |
