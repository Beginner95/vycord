...
1. `SMTP_HOST`, `SMTP_FROM`, `OTP_SECRET` (и при необходимости `SMTP_USERNAME`/`SMTP_PASSWORD`) прописаны в проде **до** раскатки — без них `config.New()` завершает процесс. С VYC-97 обязателен ещё `PHONE_ENC_KEY` (64 hex-символа, 32 байта — ключ шифрования номеров; на старте тоже fail-fast).
