...
type FriendUseCase interface {
	...
	// один из параметров непустой (гарантирует хендлер). accepted=true
	// означает, что встречная заявка уже висела и запрос её принял. self —
	// краткий профиль САМОГО вызывающего (fromID): req.User собран из target
	// и годится для HTTP-ответа вызывающему, но для WS-пуша target'у «другая
