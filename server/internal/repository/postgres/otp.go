...
func (r *otpRepository) Create(c *domain.OTPCode) error {
	...
	query := `
		INSERT INTO otp_codes (id, user_id, email, purpose, code_hash, attempts, created_at, expires_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
	`
	_, err := r.db.Exec(ctx, query, c.ID, c.UserID, c.Email, c.Purpose, c.CodeHash, c.Attempts, c.CreatedAt, c.ExpiresAt)
	if err != nil {
...
// GetActive берёт самый свежий живой код для email. ORDER BY created_at
// DESC LIMIT 1 — страховка на случай, если InvalidateActive не отработал:
// выдаём последний, а не произвольный. Ключ — email, не (email, purpose):
// на email в любой момент живёт не больше одного не сожжённого кода.
func (r *otpRepository) GetActive(email string) (*domain.OTPCode, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	query := `
		SELECT id, user_id, email, purpose, code_hash, attempts, created_at, expires_at, consumed_at, invalidated_at
		FROM otp_codes
	...
	err := r.db.QueryRow(ctx, query, email).Scan(
		&c.ID, &c.UserID, &c.Email, &c.Purpose, &c.CodeHash, &c.Attempts,
...
// verify с верным кодом выдать два результата (две сессии для
// существующего пользователя, или два новых аккаунта на один email).
...
func (r *otpRepository) InvalidateActive(email string) error {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
...
// живых кодов.
func (r *otpRepository) CountIssuedSince(email string, since time.Time) (int, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
...
func (r *otpRepository) LastIssuedAt(email string) (*time.Time, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	var last *time.Time
	err := r.db.QueryRow(ctx,
		`SELECT max(created_at) FROM otp_codes WHERE email = $1`,
		email,
	).Scan(&last)
	if err != nil {
