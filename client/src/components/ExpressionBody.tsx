...
/**
 * Вкладки и панели пикера без поверхности: `ExpressionPicker` оборачивает их в
 * поповер (`div.expression-picker` + dismiss-on-outside), мобильная шторка —
 * в BottomSheet. Всё состояние (активная вкладка, последняя вкладка) живёт
 * здесь и не зависит от того, чем обёрнуто тело.
 */
export function ExpressionBody({
  tabs, initialTab, onSelectEmoji, stickers,
}: Pick<ExpressionPickerProps, 'tabs' | 'initialTab' | 'onSelectEmoji' | 'stickers'>) {
  const t = useT();
